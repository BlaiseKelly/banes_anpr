...
The coordinates of the cameras represents the actual camera. This can affect the route selection. For this reason the points have been 'snapped' to the nearest road. The original and snapped points are shown on the map below. 
...
For OD points that are close together there can be a good degree of certainty as to the route, however for some OD points, particularly those that are far apart, the routes might not represent reality. In addition even when a most likely route has been defined, human choice/error/road conditions might mean a vehicle did not go that way.
...
certain_route <- filter(all_routes, routeID == "54_46")
uncertain_route <- filter(all_routes, routeID == "24_42")

start_pts <- filter(df_sf, id %in% c(54, 24))
end_pts <- filter(df_sf, id %in% c(46, 42))
